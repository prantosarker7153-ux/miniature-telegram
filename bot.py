...
DEFAULT_SETTINGS = {
    ...
    'folder_link_enabled': False,
    'folder_link_url': '',
    'database_channel_ids': []  # Whitelist of database channels (empty = ignore all)
}
...
def admin_main_keyboard():
    ...
    keyboard = [
        [
            ...
            InlineKeyboardButton("📢 Force Join", callback_data="admin_channels")
        ],
        [
            InlineKeyboardButton("🗄️ Database Channels", callback_data="admin_db_channels"),
...
def settings_keyboard():
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("📢 Main Channel Username", callback_data="setting_main_channel")],
        [InlineKeyboardButton("💾 Database Channels", callback_data="setting_database_channels")],  # Main feature!
...
def database_channel_keyboard():
    """Database channel manager keyboard"""
    db_channels = get_setting('database_channel_ids', [])
    main_channel = get_setting('main_channel_id')
    
    keyboard = []
    
    # Show main channel (always allowed)
    if main_channel:
        keyboard.append([
            InlineKeyboardButton(
                f"📺 Main Channel: {main_channel}", 
                callback_data="db_ch_info"
            )
        ])
    
    # Show database channels
    if db_channels:
        keyboard.append([InlineKeyboardButton("━━ Database Channels ━━", callback_data="db_ch_info")])
        for ch_id in db_channels:
            keyboard.append([
                InlineKeyboardButton(
                    f"📁 {ch_id}", 
                    callback_data=f"db_ch_{ch_id}"
                ),
                InlineKeyboardButton(
                    "❌", 
                    callback_data=f"remove_db_ch_{ch_id}"
                )
            ])
    
    keyboard.append([InlineKeyboardButton("➕ Add Database Channel", callback_data="add_db_channel")])
    keyboard.append([InlineKeyboardButton("🔙 Back", callback_data="admin_main")])
...
Select an option below:"""
        ...
        await query.edit_message_text(
            "📺 **Force Join Channel Manager**\n\nManage channels users must join to watch videos:",
            reply_markup=channel_manager_keyboard(),
            parse_mode=ParseMode.MARKDOWN
        )
    
    elif data == "admin_db_channels":
        db_channels = get_setting('database_channel_ids', [])
        main_channel = get_setting('main_channel_id')
        
        text = "🗄️ **Database Channel Manager**\n\n"
        text += "**কী করে?** এই channels থেকে video post করলে bot automatic video ID দিবে।\n\n"
        
        if main_channel:
            text += f"📺 **Main Channel:** `{main_channel}` ✅ (Always Enabled)\n\n"
        
        if db_channels:
            text += "**📁 Additional Database Channels:**\n"
            for ch_id in db_channels:
                text += f"• `{ch_id}`\n"
        else:
            text += "**📁 Additional Channels:** None\n\n"
        
        text += "\n**💡 Tip:** Main channel automatically allows video uploads. Add more channels if you have multiple content sources!"
        
        await query.edit_message_text(
            text,
            reply_markup=database_channel_keyboard(),
            parse_mode=ParseMode.MARKDOWN
        )
...
✅ Running"""
        ...
        admin_states[user_id] = {'action': 'add_database_channel'}
        await query.message.reply_text(
            "➕ **Add Database Channel**\n\n"
            "**যেই channel থেকে video নিবেন তার ID পাঠান।**\n\n"
            "📋 **Format:** `-1001234567890`\n\n"
            "**Channel ID কীভাবে পাবেন:**\n"
            "1. Channel এ @userinfobot add করুন\n"
            "2. Bot channel ID দেখাবে\n"
            "3. ID copy করে এখানে পাঠান\n\n"
            "💡 **Tip:** Bot কে channel এ admin বানাতে ভুলবেন না!\n\n"
            "Send channel ID or /cancel",
            parse_mode=ParseMode.MARKDOWN
        )
    
    elif data.startswith("remove_db_ch_"):
        ch_id = int(data.replace("remove_db_ch_", ""))
        db_channels = get_setting('database_channel_ids', [])
        
        if ch_id in db_channels:
            db_channels.remove(ch_id)
            set_setting('database_channel_ids', db_channels)
            await query.answer("✅ Database channel removed!")
        else:
    ...
    # NEW: Toggle handlers for notification settings
    
    elif data.startswith("setting_"):
        setting_key = data.replace("setting_", "")
        
        # Database Channels Setup
        if setting_key == "database_channels":
            current_db_channels = get_setting('database_channel_ids', [])
            
            if current_db_channels:
                channels_text = "\n".join([f"• `{ch_id}`" for ch_id in current_db_channels])
                channels_count = len(current_db_channels)
            else:
                channels_text = "None configured yet"
                channels_count = 0
            
            admin_states[user_id] = {'action': 'set_database_channels'}
            
            await query.message.reply_text(
                "💾 **Database Channels Setup**\n\n"
                "**🎯 Simple Whitelist System:**\n"
                "Only channels you add here will send notifications!\n\n"
                "━━━━━━━━━━━━━━━━━━━━\n\n"
                f"**Current Database Channels:** {channels_count}\n"
                f"{channels_text}\n\n"
                "━━━━━━━━━━━━━━━━━━━━\n\n"
                "**How to Add:**\n"
                "Send channel IDs separated by spaces\n\n"
                "**Example:**\n"
                "`-1001111111111 -1001222222222 -1001333333333`\n\n"
                "**💡 Result:**\n"
                "✅ Videos from THESE channels → Notification\n"
                "❌ Videos from OTHER channels → NO notification\n\n"
                "Send channel IDs or /cancel to cancel",
                parse_mode=ParseMode.MARKDOWN
            )
            return
        ...
        # NEW: Database Channels Management
        ...
        setting_map = {
            ...
            'main_channel_id': 'main_channel_id',  # NEW
            'bot_name': 'bot_name'
        }
        ...
        hints = {
            ...
            'main_channel_id': "Example: -1001234567890 (negative number)\n\nHow to get:\n1. Use @userinfobot in your channel\n2. Or check channel URL in web.telegram.org",  # NEW
...
async def channel_post(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    🎯 WHITELIST: Only channels in database list will send notifications
    """
    try:
        ...
        # 🎯 CHECK: Is this channel in database channels list?
        database_channels = get_setting('database_channel_ids', [])
        main_channel_id = get_setting('main_channel_id')
        
        # Auto-include main_channel_id in allowed channels
        allowed_channels = list(database_channels) if database_channels else []
        if main_channel_id and main_channel_id not in allowed_channels:
            allowed_channels.append(main_channel_id)
        
        if not allowed_channels:
            # No channels configured - ignore all
            logger.info(f"⏭️ No channels configured (set main_channel_id or database_channel_ids)")
            return
        
        if channel_id not in allowed_channels:
            # This channel is not in the allowed list - ignore
            logger.info(f"⏭️ Skipping channel {channel_name} (ID: {channel_id}) - Not in allowed channels")
            return
        
        # This channel is in database list - process it!
        logger.info(f"✅ Channel {channel_name} (ID: {channel_id}) is in database list - processing...")
        ...
        # Send notification with HTML format
...
async def admin_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # NEW: Handle database channels setup
    
    # Database channels setup
    if state['action'] == 'set_database_channels':
        text = text.strip()
        
        # Parse channel IDs (space or comma separated)
        ...
        for part in parts:
            try:
                ch_id = int(part)
                if ch_id < 0:  # Valid Telegram channel ID (negative)
                    channel_ids.append(ch_id)
                else:
                    errors.append(f"{part} (must be negative)")
            except ValueError:
                errors.append(f"{part} (not a number)")
        ...
        if not channel_ids:
            await update.message.reply_text(
                ...
                "Please send at least one valid channel ID or /cancel to cancel",
                parse_mode=ParseMode.MARKDOWN
            )
            return
        
        # Save database channel IDs
        set_setting('database_channel_ids', channel_ids)
        
        channels_text = "\n".join([f"• `{ch_id}`" for ch_id in channel_ids])
        
        await update.message.reply_text(
            "✅ **Database Channels Configured!**\n\n"
        ...
        del admin_states[user_id]
        logger.info(f"✅ Database channels configured: {len(channel_ids)} channels")
        return
    
    if state['action'] == 'add_database_channel':
        try:
            channel_id = int(text.strip())
            db_channels = get_setting('database_channel_ids', [])
            
            if channel_id in db_channels:
                await update.message.reply_text("⚠️ This channel is already in the database list!")
                return
            ...
            await update.message.reply_text(
                f"✅ **Database Channel Added!**\n\n"
                f"**Channel ID:** `{channel_id}`\n\n"
                f"এখন এই channel এ video post করলে আপনি automatic notification পাবেন!\n\n"
                f"💡 **Remember:** Bot কে channel এ admin বানাতে ভুলবেন না!",
                parse_mode=ParseMode.MARKDOWN
            )
            del admin_states[user_id]
        except ValueError:
